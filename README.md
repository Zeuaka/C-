...
**Алгоритм решения:**  
В методе сравнивается число с границами `a` и `b`, так же проводится второе сравнение, но уже меняя эти границы местами. Если хотя бы в одном сравнении число оказалось попадающим в промежуток, метод выводит `true`, иначе `false`.  
...
**Алгоритм решения:**  
Метод выводит результат пересечения двух условий: `a = b && a = c`.  
...
**Алгоритм решения:**  
Если число больше нуля, метод возвращает само число, иначе умножает его на -1.  
...
**Алгоритм решения:**  
Метод проверяет следующее условие: если сумма остатка от деления `x` на 3 + остатка от деления `x` на 5 `!= 0` и только один из остатков от деления (на 5 или на 3) равен 0.  
...
**Алгоритм решения:**  
Метод возвращает 20, если сумма двух чисел в диапазоне от 11 до 19 (включительно), иначе возвращает их сумму.  
...
**Алгоритм решения:**  
Метод реализован с помощью стандартного прохода цикла `for (i от 0 до x)` и вывода параметра `i` на каждой итерации цикла.  
...
**Алгоритм решения:**  
Метод реализован с помощью стандартного прохода цикла `for (i от 0 до x)` и вывода параметра `i` на каждой итерации цикла, при условии что `i` — четное число.  
...
**Алгоритм решения:**  
Метод реализован с помощью цикла `while (x > 0)`. На каждом шаге цикла число `x` укорачивается на один разряд, а счетчик увеличивается на единицу. В конце работы возвращается значение счетчика.  
...
**Алгоритм решения:**  
Метод реализован с помощью двух циклов, где один вложен в другой. Оба цикла ограничены `x`. На каждой итерации внутреннего цикла выводится `*`, на каждой итерации внешнего происходит перенос строки.  
...
**Алгоритм решения:**  
Метод реализован с помощью двух циклов, где один вложен в другой. Оба цикла ограничены `x`. Также в методе предусматривается отдельный указатель `s = x - 1`. Так, при каждой итерации внутреннего цикла, если `i >= s`, выводится `*`, иначе выводится пробел. При каждой итерации внешнего цикла `s` уменьшается на 1, а строка переносится на следующую.  

---

### **МАССИВЫ**  

В каждой задаче этого раздела пользователю предлагается создавать массивы тремя различными способами с помощью отдельного метода.  
...
**Алгоритм решения:**  
С помощью цикла `for` осуществляется проход по всем элементам массива. Если во время этого прохода находится такой элемент массива, который равен `x`, выводится его индекс.  
...
**Алгоритм решения:**  
Задается начальное значение максимума, указывающее на первый элемент массива. С помощью цикла `for` осуществляется проход по всем элементам массива. Если находится элемент, больший максимума по модулю (модуль числа вычисляется с помощью метода `abs`), максимум перезаписывается на этот элемент. В конце работы метод возвращает значение максимума.  
...
**Алгоритм решения:**  
Задается значение `int counter = 0`, оно будет ориентиром для добавления в новый массив. Также создается массив `int[] mix = new int[arr.Length + ins.Length]`, количество ячеек в котором будет равно количеству ячеек в двух исходных массивах вместе. Далее с помощью цикла `for` осуществляется проход по всем элементам первого массива, и благодаря заданному `counter` индекс финального массива не теряется. Каждую итерацию цикла происходит присваивание `mix[counter] = arr[i]`, а `counter` увеличивается на 1. Далее такая же процедура происходит и со вторым массивом. В конце работы метод возвращает итоговый массив `mix`.  
...
**Алгоритм решения:**  
Создается новый массив `arr1`, в котором количество ячеек равно количеству в исходном массиве `arr`. Далее задается стартовая позиция для нового массива `int j = 0`. После чего происходит обратный проход исходного массива с помощью цикла `for (int i = arr.Length - 1; i >= 0; i--)`, где на каждой итерации происходит присвоение `arr1[j] = arr[i]`, а счетчик `j` увеличивается на 1.  
...
**Алгоритм решения:**  
1. Проходим по всему массиву `arr` с помощью цикла `for`.  
2. Считается, сколько раз элемент `x` встречается в массиве. Результат сохраняется в переменную `counter`.  
3. Создается новый массив `newarr` размером `counter`.  
4. Снова проходим по исходному массиву. При нахождении элемента `x` записываем его индекс в `newarr`. Используем переменную `lenght` для навигации, каждую итерацию она будет увеличиваться на 1.  
5. В конце работы метод возвращает `newarr`.  
